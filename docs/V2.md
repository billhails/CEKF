...
| bytecode | condition | new state |
|----------|-----------|--------|
| `\| aexp \| IF \| addr(ef) \| et \| ef \|` | `pop(p)` | `(addr(et), E, K, F)` |
|  | `!pop(p)` | `(addr(ef), E, K, F)` |
...
| bytecode | new state |
|----|----|
| `\| AMB \| addr(e2) \| e1 \| e2 \|` | `(addr(e1), E, K, backtrack(addr(e2), E, K, F))` |
...
count minus its lexical depth (the number of `let`s and `letrec`s between it and the top-level of its function) is zero.
