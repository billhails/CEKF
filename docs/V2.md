...
Three bytes, a `VAR` tag that identifies that a variable is coming
up, then a byte for its frame and a byte for its index (see [Lexical
Addressing](LEXICAL_ADDRESSING.md) for details if you haven't already.)

#### action

On seeing that, the bytecode interpreter does:

```
push(lookup(frame, offset, env))
```
...
| bytecode | condition | action |
|----------|-----------|--------|
| `| aexp1 | ... | aexpn | aexp | APPLY |` | `clo(nargs, addr, env) = pop()` | `env' = extend(env, pop(nargs));`<br/>`(addr, env', K, F) |
