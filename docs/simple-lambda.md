...
1. `let*` becomes a nest of `let`. **done**
1. `let` in turn becomes an anonymous lambda. **done**
...
1. `Construct` replaced with simple vector operation.  (`constructToMakeVec` in `anf_normalize.c`). **done**
1. `Deconstruct` replaced with simple vector operation.  (`deconstructToPrimApp` in `anf_normalize.c`). **done**
