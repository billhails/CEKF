...
**Design Decision**: All continuation-specific code (declarations, wrappers, constructors) goes in a separate `.inc` file that is included only by `anf_normalize_2.c`. Everything in this file is declared `static` for maximum encapsulation.

Note that rather than overwriting the existing implementation we are creating this new `anf_normalize_2.c` as a fresh file that is allowed to be broken.
...
Here's how all the pieces fit together in `anf_normalize_2.c`:
...
**Design Decision**: Wrappers are included in `anf_normalize_2.c` via the `.inc` file:

```c
// generated/anf_kont_impl.inc (included by anf_normalize_2.c)
...
2. **`generated/anf_kont_impl.inc`** (new special codegen)
   ...
   - All `static` - private to anf_normalize_2.c
   - Included only by anf_normalize_2.c
...
- `anf_normalize_2.c`: Includes both, implements continuation bodies, exposes single public `anfNormalize()` function
...
#### F. User Implementation Pattern in anf_normalize_2.c
```c
...
**Note**: Constructors are public API - declared in header, implemented in .inc, used throughout anf_normalize_2.c
...
- Implemented in generated `.inc` file (included by anf_normalize_2.c)
