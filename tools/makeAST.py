...
class SimpleArray(Base):
    ...
    def printPeeknFunction(self, catalog):
        if self.dimension == 1:
            ...
            print(f'        cant_happen("stack overflow %d/%u", offset, x->size); {c}')
            print(f"    }} {c}")
...
class SimpleStack(SimpleArray):
    ...
    def printSizeDeclaration(self, catalog):
        ...
        print(f"static inline Index frameSize{name}({myType} {a}obj) {{ return obj->frames_size; }} {c}")
        print(f"static inline Index offset{name}({myType} {a}obj) {{ return obj->offset; }} {c}")

    def printPushDeclaration(self, catalog):
        super().printPushDeclaration(catalog)
        name = self.getName()
        myType = self.getTypeDeclaration(catalog)
        entryType = self.entries.getTypeDeclaration(catalog)
        a = '*' if self.isInline(catalog) else ''
        c = self.comment('printPushDeclaration')
        print(f"Index pushFrame{name}({myType} {a}obj); {c}")

    def printPopDeclaration(self, catalog):
        super().printPopDeclaration(catalog);
        name = self.getName()
        myType = self.getTypeDeclaration(catalog)
        entryType = self.entries.getTypeDeclaration(catalog)
        a = '*' if self.isInline(catalog) else ''
        c = self.comment('printPopDeclaration')
        print(f"void popFrame{name}({myType} {a}obj); {c}")
    ...
    def printPushFunction(self, catalog):
        ...
        print(f" * Returns the stack pointer after the push.")
        print(f" */")
        print(f"Index push{name}Entries({myType} {a}x, {entryType} entry) {{ {c}")
        print(f'    DEBUG("push{name}Entries(%p)", x);')
        ...
        print(f"")
        print(f"/**")
        print(f" * Pushes a new frame on to `x`, extending `x` if required.")
        print(f" * Returns the frame pointer after the push.")
        print(f" */")
        print(f"Index push{name}Frames({myType} {a}x) {{ {c}")
        print(f'    DEBUG("push{name}Frames(%p)", x);')
        print(f"    extend{name}Frames(x, x->frames_size + 1); {c}")
        print(f"    x->frames[x->frames_size++] = (StackFrame) {{.frame = x->frame, .offset = x->offset }}; {c}")
        print(f"    x->frame += x->offset; {c}")
        print(f"    x->offset = 0; {c}")
        print(f"    return x->frame; {c}")
        print(f"}} {c}")
        print(f"")
    ...
    def printPopFunction(self, catalog):
        ...
        print(f'    DEBUG("pop{name}(%p)", x);')
        print(f"#ifdef SAFETY_CHECKS")
        print(f"    if (x->frames_size == 0) {{ {c}")
        print(f'        cant_happen("stack underflow"); {c}')
        print(f"    }} {c}")
        print(f"#endif")
        print(f"    x->frame = x->frames[x->frames_size - 1].frame; {c}")
        print(f"    x->offset = x->frames[x->frames_size - 1].offset; {c}")
        print(f"    x->frames_size--; {c}")
        print(f"}} {c}")
        print(f"")
    ...
    def printCopyEntriesFunction(self, catalog):
        ...
        print(f"    extend{name}Frames(dest, src->frames_size); {c}")
        print(f"    COPY_ARRAY({entryType}, dest->entries, src->entries, src->frame + src->offset); {c}")
        print(f"    COPY_ARRAY(StackFrame, dest->frames, src->frames, src->frames_size); {c}")
        print(f"    dest->frames_size = src->frames_size; {c}")
    ...
    def printCountDeclaration(self, catalog):
        ...
        print(f'    return x->frames_size; {c}')
        print(f'}} {c}')
        print('')
    ...
    def printCompareFunction(self, catalog):
        ...
        print(f"    if (a->frames_size != b->frames_size) return false; {c}")
        print(f"    for (Index i = 0; i < a->frames_size; i++) {{ {c}")
    ...
    def print1dCopyFunctionBody(self, catalog):
        ...
        print(f"        extend{myName}Frames(x, o->frames_size); {c}")
        print(f"        COPY_ARRAY(StackFrame, x->frames, o->frames, o->frames_size); {c}")
        print(f"        x->frames_size = o->frames_size; {c}")
        print(f"    }} {c}")
    ...
    def printPrintFunction(self, catalog):
        ...
        print(f'    eprintf("{myName}(%d)[\\n", x->frames_size); {c}')
        print(f"    for (Index i = 0; i < x->frames_size; i++) {{ {c}")
