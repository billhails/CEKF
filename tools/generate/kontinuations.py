...
""")
    ...
    def _write_forward_decls(self, output: TextIO, static=False) -> None:
        ...
        for name, spec in self.continuations.items():
            ...
            output.write(f"{modifier}{self._get_output_type()} *{name}Kont({self._get_input_type()}*, {struct_name}*);\n")
        output.write("\n")
    ...
    def _write_wrappers(self, output: TextIO, static=False) -> None:
        ...
        for name, spec in self.continuations.items():
            ...
            output.write(f"{modifier}struct {self._get_output_type()} *{name}Wrapper(")
            output.write(f"struct {self._get_input_type()} *exp, ")
    ...
    def _get_input_type(self) -> str:
        """Get the input type (the type passed through continuations) from config"""
        if 'input' not in self.config:
            return self._get_currency_type()
        return self.config['input']

    def _get_output_type(self) -> str:
        """Get the output type (the type passed through continuations) from config"""
    ...
    def generate_kont_impl_h(self, output: TextIO, catalog: Catalog, includes) -> None:
        ...
        for cont_name, spec in self.continuations.items():
            if spec.get('notimplemented', False):
                continue
            output.write(f"struct {self._get_output_type()} *{cont_name}Wrapper(")
            output.write(f"struct {self._get_input_type()} *exp, ")
            output.write(f"struct {kont_env_union} *env);\n")
        output.write("\n")
