...
#define NEW_VECTOR(size, type, element_type, tag)                              \
    ((type *)allocate(sizeof(type) + (size) * sizeof(element_type), (tag)))
#define FREE_VECTOR(vector, type, element_type, count)                         \
    ((void)reallocate(vector, sizeof(type) + (count) * sizeof(element_type), 0))

#define NEW_VEC(size)                                                          \
    ((Vec *)allocate(sizeof(Vec) + size * sizeof(Value), OBJTYPE_VEC))
#define FREE_VEC(vec)                                                          \
    ((void)reallocate(vec, sizeof(vec) + vec->size * sizeof(Value), 0))
...
#define FREE_ARRAY(type, array, count)                                         \
    ((void)reallocate(array, sizeof(type) * (count), 0))
#define GROW_ARRAY(type, array, oldcount, newcount)                            \
    ((type *)reallocate(array, sizeof(type) * (oldcount),                      \
                        sizeof(type) * (newcount)))
#define MOVE_ARRAY(type, dest, src, amount)                                    \
    (memmove((dest), (src), sizeof(type) * (amount)))
#define COPY_ARRAY(type, dest, src, amount)                                    \
    (memcpy((dest), (src), sizeof(type) * (amount)))
