...
bigint_word bigint_word_mul_hi(bigint_word a, bigint_word b) {
    ...
    bigint_word c4 =
        BIGINT_WORD_HI(c0) + BIGINT_WORD_LO(c1) + BIGINT_WORD_LO(c2);
...
int bigint_raw_cmp_abs(const bigint_word *a, int na, const bigint_word *b,
                       int nb) {
    int i;
...
int bigint_raw_cmp(const bigint_word *a, int na, int a_neg,
...
int bigint_raw_mul_word_add(bigint_word *dst, const bigint_word *src, int n,
                            bigint_word factor) {
    int i;
    bigint_word carry = 0;
...
int bigint_raw_mul_word(bigint_word *dst, const bigint_word *src, int n,
                        bigint_word factor) {
    int i;
    bigint_word carry = 0;
...
int bigint_raw_mul_add(bigint_word *dst, const bigint_word *src_a, int na,
...
int bigint_raw_add_word(bigint_word *dst, const bigint_word *src, int n,
                        bigint_word b) {
    int i;
    bigint_word carry = b;
...
int bigint_raw_add(bigint_word *dst, const bigint_word *src_a, int na,
                   const bigint_word *src_b, int nb) {
...
int bigint_raw_sub(bigint_word *dst, const bigint_word *src_a, int na,
                   const bigint_word *src_b, int nb) {
...
int bigint_raw_mul_karatsuba(bigint_word *dst, const bigint_word *a, int na,
                             const bigint_word *b, int nb, bigint_word *tmp) {
    /* so many */
    int n, k, m, m2;
    ...
    n = bigint_raw_add(dst + m2 * 1, dst + m2 * 1, BIGINT_MAX(n - m2, 0), z1,
                       nz1);
    n = bigint_raw_add(dst + m2 * 2, dst + m2 * 2, BIGINT_MAX(n - m2, 0), z2,
                       nz2);
...
int bigint_raw_shift_left(bigint_word *dst, int n_dst, const bigint_word *src,
                          int n_src, unsigned shift) {
    int i;
    ...
    if (bits_shift) {
        ...
        for (i = n_src + word_shift; i > word_shift; i--) {
            ...
            dst[i] =
                (hi << bits_shift) | (lo >> (BIGINT_WORD_BITS - bits_shift));
            hi = lo;
        }
...
int bigint_raw_shift_right(bigint_word *dst, int n_dst,
    ...
    if (bits_shift) {
        ...
        for (i = 0; i < n_src - word_shift - 1; i++) {
            ...
            BIGINT_ASSERT(i, <, n_dst);
            dst[i] =
                (hi << (BIGINT_WORD_BITS - bits_shift)) | (lo >> bits_shift);
            lo = hi;
        }
...
int bigint_count_trailing_zeros(const bigint * a) {
    ...
    for (i = 0; i < a->size; i++) {
        ...
        if (w)
            return bigint_word_count_trailing_zeros(w) + i * BIGINT_WORD_BITS;
    }
...
bigint *bigint_gcd(bigint * dst, const bigint * src_a, const bigint * src_b) {
    ...
    if (src_a->size == 0)
        return bigint_set_neg(bigint_cpy(dst, src_b), 0);
    if (src_b->size == 0)
        return bigint_set_neg(bigint_cpy(dst, src_a), 0);
...
int bigint_is_probable_prime(const bigint * n, int n_tests,
    ...
    if (bigint_get_bit(n, 0) == 0)
        return 0;

    /* 1, 3 are prime */
    if (bigint_cmp_abs_word(n, 3) <= 0)
        return 1;
    ...
    do {
        ...
        if (bigint_cmp_abs_word(x, 1) == 0)
            continue;
        if (bigint_cmp(x, n_minus_one) == 0)
            continue;
...
void bigint_raw_get_high_bits(bigint_word *dst, int n_dst,
                              const bigint_word *src_a, int na, int n_bits,
                              int *n_bitlength) {
    int shift;
...
Cmp cmpMaybeBigInt(MaybeBigInt *x, MaybeBigInt *y) {
    switch (x->type) {
        case BI_SMALL:
            switch (y->type) {
                case BI_SMALL:
                    return x->small < y->small ?  -1 :
                           x->small == y->small ?  0 : 1;
                case BI_BIG:
                    bigint bx;
                    bigint_init(&bx);
                    bigint_from_int(&bx, x->small);
                    Cmp res = (Cmp) bigint_cmp(&bx, &y->big);
                    bigint_free(&bx);
                    return res;
                case BI_IRRATIONAL:
                    return x->small < y->irrational ?  -1 :
                           x->small == y->irrational ?  0 : 1;
                default:
                    cant_happen("unrecognized type of MaybeBigInt: %d", x->type);
            }
            break;
        ...
        case BI_IRRATIONAL:
            switch (y->type) {
                case BI_SMALL:
                    return x->irrational < y->small ?  -1 :
                           x->irrational == y->small ?  0 : 1;
                case BI_BIG:
                    cant_happen("attempt to compare bigint and rational");
                case BI_IRRATIONAL:
                    return x->irrational < y->irrational ?  -1 :
                           x->irrational == y->irrational ?  0 : 1;
                default:
                    cant_happen("unrecognized type of MaybeBigInt: %d", x->type);
            }
            break;
