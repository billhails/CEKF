...
struct LamLam * copyLamLam(struct LamLam * o) {
    if (o == NULL) return NULL;
    struct LamLam * x = NEW(LamLam, OBJTYPE_LAMLAM);
    DEBUG("copy LamLam %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamLam));
    x->header = _h;
    int save = PROTECT(x);
    x->nargs = o->nargs;
    x->args = copyLamVarList(o->args);
    x->exp = copyLamExp(o->exp);
    UNPROTECT(save);
    return x;
}

struct LamVarList * copyLamVarList(struct LamVarList * o) {
    if (o == NULL) return NULL;
    struct LamVarList * x = NEW(LamVarList, OBJTYPE_LAMVARLIST);
    DEBUG("copy LamVarList %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamVarList));
    x->header = _h;
    int save = PROTECT(x);
    x->var = o->var;
    x->next = copyLamVarList(o->next);
    UNPROTECT(save);
    return x;
}

struct LamPrimApp * copyLamPrimApp(struct LamPrimApp * o) {
    if (o == NULL) return NULL;
    struct LamPrimApp * x = NEW(LamPrimApp, OBJTYPE_LAMPRIMAPP);
    DEBUG("copy LamPrimApp %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamPrimApp));
    x->header = _h;
    int save = PROTECT(x);
    x->type = o->type;
    x->exp1 = copyLamExp(o->exp1);
    x->exp2 = copyLamExp(o->exp2);
    UNPROTECT(save);
    return x;
}

struct LamUnaryApp * copyLamUnaryApp(struct LamUnaryApp * o) {
    if (o == NULL) return NULL;
    struct LamUnaryApp * x = NEW(LamUnaryApp, OBJTYPE_LAMUNARYAPP);
    DEBUG("copy LamUnaryApp %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamUnaryApp));
    x->header = _h;
    int save = PROTECT(x);
    x->type = o->type;
    x->exp = copyLamExp(o->exp);
...
struct LamSequence * copyLamSequence(struct LamSequence * o) {
    if (o == NULL) return NULL;
    struct LamSequence * x = NEW(LamSequence, OBJTYPE_LAMSEQUENCE);
    DEBUG("copy LamSequence %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamSequence));
    x->header = _h;
    int save = PROTECT(x);
    x->exp = copyLamExp(o->exp);
    x->next = copyLamSequence(o->next);
    UNPROTECT(save);
    return x;
}

struct LamList * copyLamList(struct LamList * o) {
    if (o == NULL) return NULL;
    struct LamList * x = NEW(LamList, OBJTYPE_LAMLIST);
    DEBUG("copy LamList %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamList));
    x->header = _h;
    int save = PROTECT(x);
    x->exp = copyLamExp(o->exp);
    x->next = copyLamList(o->next);
    UNPROTECT(save);
    return x;
}

struct LamApply * copyLamApply(struct LamApply * o) {
    if (o == NULL) return NULL;
    struct LamApply * x = NEW(LamApply, OBJTYPE_LAMAPPLY);
    DEBUG("copy LamApply %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamApply));
    x->header = _h;
    int save = PROTECT(x);
    x->function = copyLamExp(o->function);
    x->nargs = o->nargs;
    x->args = copyLamList(o->args);
    UNPROTECT(save);
    return x;
}

struct LamConstant * copyLamConstant(struct LamConstant * o) {
    if (o == NULL) return NULL;
    struct LamConstant * x = NEW(LamConstant, OBJTYPE_LAMCONSTANT);
    DEBUG("copy LamConstant %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamConstant));
    x->header = _h;
    int save = PROTECT(x);
    x->name = o->name;
    x->tag = o->tag;
...
struct LamConstruct * copyLamConstruct(struct LamConstruct * o) {
    if (o == NULL) return NULL;
    struct LamConstruct * x = NEW(LamConstruct, OBJTYPE_LAMCONSTRUCT);
    DEBUG("copy LamConstruct %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamConstruct));
    x->header = _h;
    int save = PROTECT(x);
    x->name = o->name;
    x->tag = o->tag;
...
struct LamDeconstruct * copyLamDeconstruct(struct LamDeconstruct * o) {
    if (o == NULL) return NULL;
    struct LamDeconstruct * x = NEW(LamDeconstruct, OBJTYPE_LAMDECONSTRUCT);
    DEBUG("copy LamDeconstruct %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamDeconstruct));
    x->header = _h;
    int save = PROTECT(x);
    x->name = o->name;
    x->vec = o->vec;
...
struct LamMakeVec * copyLamMakeVec(struct LamMakeVec * o) {
    if (o == NULL) return NULL;
    struct LamMakeVec * x = NEW(LamMakeVec, OBJTYPE_LAMMAKEVEC);
    DEBUG("copy LamMakeVec %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamMakeVec));
    x->header = _h;
    int save = PROTECT(x);
    x->nargs = o->nargs;
    x->args = copyLamList(o->args);
...
struct LamIff * copyLamIff(struct LamIff * o) {
    if (o == NULL) return NULL;
    struct LamIff * x = NEW(LamIff, OBJTYPE_LAMIFF);
    DEBUG("copy LamIff %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamIff));
    x->header = _h;
    int save = PROTECT(x);
    x->condition = copyLamExp(o->condition);
    x->consequent = copyLamExp(o->consequent);
    x->alternative = copyLamExp(o->alternative);
    UNPROTECT(save);
    return x;
}

struct LamCond * copyLamCond(struct LamCond * o) {
    if (o == NULL) return NULL;
    struct LamCond * x = NEW(LamCond, OBJTYPE_LAMCOND);
    DEBUG("copy LamCond %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamCond));
    x->header = _h;
    int save = PROTECT(x);
    x->value = copyLamExp(o->value);
    x->cases = copyLamCondCases(o->cases);
    UNPROTECT(save);
    return x;
}

struct LamIntCondCases * copyLamIntCondCases(struct LamIntCondCases * o) {
    if (o == NULL) return NULL;
    struct LamIntCondCases * x = NEW(LamIntCondCases, OBJTYPE_LAMINTCONDCASES);
    DEBUG("copy LamIntCondCases %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamIntCondCases));
    x->header = _h;
    int save = PROTECT(x);
    x->constant = o->constant;
    x->body = copyLamExp(o->body);
    x->next = copyLamIntCondCases(o->next);
    UNPROTECT(save);
    return x;
}

struct LamCharCondCases * copyLamCharCondCases(struct LamCharCondCases * o) {
    if (o == NULL) return NULL;
    struct LamCharCondCases * x = NEW(LamCharCondCases, OBJTYPE_LAMCHARCONDCASES);
    DEBUG("copy LamCharCondCases %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamCharCondCases));
    x->header = _h;
    int save = PROTECT(x);
    x->constant = o->constant;
    x->body = copyLamExp(o->body);
    x->next = copyLamCharCondCases(o->next);
    UNPROTECT(save);
    return x;
}

struct LamMatch * copyLamMatch(struct LamMatch * o) {
    if (o == NULL) return NULL;
    struct LamMatch * x = NEW(LamMatch, OBJTYPE_LAMMATCH);
    DEBUG("copy LamMatch %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamMatch));
    x->header = _h;
    int save = PROTECT(x);
    x->index = copyLamExp(o->index);
    x->cases = copyLamMatchList(o->cases);
    UNPROTECT(save);
    return x;
}

struct LamMatchList * copyLamMatchList(struct LamMatchList * o) {
    if (o == NULL) return NULL;
    struct LamMatchList * x = NEW(LamMatchList, OBJTYPE_LAMMATCHLIST);
    DEBUG("copy LamMatchList %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamMatchList));
    x->header = _h;
    int save = PROTECT(x);
    x->matches = copyLamIntList(o->matches);
    x->body = copyLamExp(o->body);
    x->next = copyLamMatchList(o->next);
    UNPROTECT(save);
    return x;
}

struct LamIntList * copyLamIntList(struct LamIntList * o) {
    if (o == NULL) return NULL;
    struct LamIntList * x = NEW(LamIntList, OBJTYPE_LAMINTLIST);
    DEBUG("copy LamIntList %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamIntList));
    x->header = _h;
    int save = PROTECT(x);
    x->item = o->item;
    x->name = o->name;
    x->next = copyLamIntList(o->next);
    UNPROTECT(save);
    return x;
}

struct LamLet * copyLamLet(struct LamLet * o) {
    if (o == NULL) return NULL;
    struct LamLet * x = NEW(LamLet, OBJTYPE_LAMLET);
    DEBUG("copy LamLet %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamLet));
    x->header = _h;
    int save = PROTECT(x);
    x->var = o->var;
    x->value = copyLamExp(o->value);
...
struct LamLetRec * copyLamLetRec(struct LamLetRec * o) {
    if (o == NULL) return NULL;
    struct LamLetRec * x = NEW(LamLetRec, OBJTYPE_LAMLETREC);
    DEBUG("copy LamLetRec %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamLetRec));
    x->header = _h;
    int save = PROTECT(x);
    x->nbindings = o->nbindings;
    x->bindings = copyLamLetRecBindings(o->bindings);
    x->body = copyLamExp(o->body);
    UNPROTECT(save);
    return x;
}

struct LamLetRecBindings * copyLamLetRecBindings(struct LamLetRecBindings * o) {
    if (o == NULL) return NULL;
    struct LamLetRecBindings * x = NEW(LamLetRecBindings, OBJTYPE_LAMLETRECBINDINGS);
    DEBUG("copy LamLetRecBindings %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamLetRecBindings));
    x->header = _h;
    int save = PROTECT(x);
    x->var = o->var;
    x->val = copyLamExp(o->val);
    x->next = copyLamLetRecBindings(o->next);
    UNPROTECT(save);
    return x;
}

struct LamContext * copyLamContext(struct LamContext * o) {
    if (o == NULL) return NULL;
    struct LamContext * x = NEW(LamContext, OBJTYPE_LAMCONTEXT);
    DEBUG("copy LamContext %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamContext));
    x->header = _h;
    int save = PROTECT(x);
    x->frame = o->frame;
    x->parent = copyLamContext(o->parent);
    UNPROTECT(save);
    return x;
}
...
struct LamAnd * copyLamAnd(struct LamAnd * o) {
    if (o == NULL) return NULL;
    struct LamAnd * x = NEW(LamAnd, OBJTYPE_LAMAND);
    DEBUG("copy LamAnd %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamAnd));
    x->header = _h;
    int save = PROTECT(x);
    x->left = copyLamExp(o->left);
    x->right = copyLamExp(o->right);
    UNPROTECT(save);
    return x;
}
...
struct LamOr * copyLamOr(struct LamOr * o) {
    if (o == NULL) return NULL;
    struct LamOr * x = NEW(LamOr, OBJTYPE_LAMOR);
    DEBUG("copy LamOr %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamOr));
    x->header = _h;
    int save = PROTECT(x);
    x->left = copyLamExp(o->left);
    x->right = copyLamExp(o->right);
...
struct LamAmb * copyLamAmb(struct LamAmb * o) {
    if (o == NULL) return NULL;
    struct LamAmb * x = NEW(LamAmb, OBJTYPE_LAMAMB);
    DEBUG("copy LamAmb %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamAmb));
    x->header = _h;
    int save = PROTECT(x);
    x->left = copyLamExp(o->left);
    x->right = copyLamExp(o->right);
...
struct LamPrint * copyLamPrint(struct LamPrint * o) {
    ...
    int save = PROTECT(x);
    x->exp = copyLamExp(o->exp);
    x->printer = copyLamExp(o->printer);
    UNPROTECT(save);
    return x;
}
...
struct LamTypeDefs * copyLamTypeDefs(struct LamTypeDefs * o) {
    if (o == NULL) return NULL;
    struct LamTypeDefs * x = NEW(LamTypeDefs, OBJTYPE_LAMTYPEDEFS);
    DEBUG("copy LamTypeDefs %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeDefs));
    x->header = _h;
    int save = PROTECT(x);
    x->typeDefs = copyLamTypeDefList(o->typeDefs);
    x->body = copyLamExp(o->body);
    UNPROTECT(save);
    return x;
}
...
struct LamTypeDefList * copyLamTypeDefList(struct LamTypeDefList * o) {
    if (o == NULL) return NULL;
    struct LamTypeDefList * x = NEW(LamTypeDefList, OBJTYPE_LAMTYPEDEFLIST);
    DEBUG("copy LamTypeDefList %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeDefList));
    x->header = _h;
    int save = PROTECT(x);
    x->typeDef = copyLamTypeDef(o->typeDef);
    x->next = copyLamTypeDefList(o->next);
    UNPROTECT(save);
    return x;
}
...
struct LamTypeDef * copyLamTypeDef(struct LamTypeDef * o) {
    if (o == NULL) return NULL;
    struct LamTypeDef * x = NEW(LamTypeDef, OBJTYPE_LAMTYPEDEF);
    DEBUG("copy LamTypeDef %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeDef));
    x->header = _h;
    int save = PROTECT(x);
    x->type = copyLamType(o->type);
    x->constructors = copyLamTypeConstructorList(o->constructors);
    UNPROTECT(save);
    return x;
}

struct LamTypeConstructorList * copyLamTypeConstructorList(struct LamTypeConstructorList * o) {
    if (o == NULL) return NULL;
    struct LamTypeConstructorList * x = NEW(LamTypeConstructorList, OBJTYPE_LAMTYPECONSTRUCTORLIST);
    DEBUG("copy LamTypeConstructorList %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeConstructorList));
    x->header = _h;
    int save = PROTECT(x);
    x->constructor = copyLamTypeConstructor(o->constructor);
    x->next = copyLamTypeConstructorList(o->next);
    UNPROTECT(save);
    return x;
}

struct LamType * copyLamType(struct LamType * o) {
    if (o == NULL) return NULL;
    struct LamType * x = NEW(LamType, OBJTYPE_LAMTYPE);
    DEBUG("copy LamType %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamType));
    x->header = _h;
    int save = PROTECT(x);
    x->name = o->name;
    x->args = copyLamTypeArgs(o->args);
    UNPROTECT(save);
    return x;
}

struct LamTypeArgs * copyLamTypeArgs(struct LamTypeArgs * o) {
    if (o == NULL) return NULL;
    struct LamTypeArgs * x = NEW(LamTypeArgs, OBJTYPE_LAMTYPEARGS);
    DEBUG("copy LamTypeArgs %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeArgs));
    x->header = _h;
    int save = PROTECT(x);
    x->name = o->name;
    x->next = copyLamTypeArgs(o->next);
    UNPROTECT(save);
    return x;
}

struct LamTypeConstructor * copyLamTypeConstructor(struct LamTypeConstructor * o) {
    if (o == NULL) return NULL;
    struct LamTypeConstructor * x = NEW(LamTypeConstructor, OBJTYPE_LAMTYPECONSTRUCTOR);
    DEBUG("copy LamTypeConstructor %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeConstructor));
    x->header = _h;
    int save = PROTECT(x);
    x->name = o->name;
    x->type = copyLamType(o->type);
    x->args = copyLamTypeConstructorArgs(o->args);
    UNPROTECT(save);
    return x;
}

struct LamTypeConstructorArgs * copyLamTypeConstructorArgs(struct LamTypeConstructorArgs * o) {
    if (o == NULL) return NULL;
    struct LamTypeConstructorArgs * x = NEW(LamTypeConstructorArgs, OBJTYPE_LAMTYPECONSTRUCTORARGS);
    DEBUG("copy LamTypeConstructorArgs %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeConstructorArgs));
    x->header = _h;
    int save = PROTECT(x);
    x->arg = copyLamTypeConstructorType(o->arg);
    x->next = copyLamTypeConstructorArgs(o->next);
    UNPROTECT(save);
    return x;
}

struct LamTypeFunction * copyLamTypeFunction(struct LamTypeFunction * o) {
    if (o == NULL) return NULL;
    struct LamTypeFunction * x = NEW(LamTypeFunction, OBJTYPE_LAMTYPEFUNCTION);
    DEBUG("copy LamTypeFunction %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeFunction));
    x->header = _h;
    int save = PROTECT(x);
    x->name = o->name;
    x->args = copyLamTypeConstructorArgs(o->args);
...
struct LamTypeConstructorInfo * copyLamTypeConstructorInfo(struct LamTypeConstructorInfo * o) {
    if (o == NULL) return NULL;
    struct LamTypeConstructorInfo * x = NEW(LamTypeConstructorInfo, OBJTYPE_LAMTYPECONSTRUCTORINFO);
    DEBUG("copy LamTypeConstructorInfo %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeConstructorInfo));
    x->header = _h;
    int save = PROTECT(x);
    x->type = copyLamTypeConstructor(o->type);
    x->vec = o->vec;
    x->arity = o->arity;
    x->size = o->size;
    x->index = o->index;
...
struct LamExp * copyLamExp(struct LamExp * o) {
    if (o == NULL) return NULL;
    struct LamExp * x = NEW(LamExp, OBJTYPE_LAMEXP);
    DEBUG("copy LamExp %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamExp));
    x->header = _h;
    int save = PROTECT(x);
    switch(o->type) {
        case LAMEXP_TYPE_LAM:
            x->val.lam = copyLamLam(o->val.lam);
            break;
        case LAMEXP_TYPE_VAR:
            x->val.var = o->val.var;
            break;
        case LAMEXP_TYPE_STDINT:
            x->val.stdint = o->val.stdint;
            break;
        case LAMEXP_TYPE_BIGINTEGER:
            x->val.biginteger = o->val.biginteger;
            break;
        case LAMEXP_TYPE_PRIM:
            x->val.prim = copyLamPrimApp(o->val.prim);
            break;
        case LAMEXP_TYPE_UNARY:
            x->val.unary = copyLamUnaryApp(o->val.unary);
            break;
        case LAMEXP_TYPE_LIST:
            x->val.list = copyLamSequence(o->val.list);
            break;
        case LAMEXP_TYPE_MAKEVEC:
            x->val.makeVec = copyLamMakeVec(o->val.makeVec);
            break;
        case LAMEXP_TYPE_CONSTRUCT:
            x->val.construct = copyLamConstruct(o->val.construct);
            break;
        case LAMEXP_TYPE_DECONSTRUCT:
            x->val.deconstruct = copyLamDeconstruct(o->val.deconstruct);
            break;
        case LAMEXP_TYPE_CONSTANT:
            x->val.constant = copyLamConstant(o->val.constant);
            break;
        case LAMEXP_TYPE_APPLY:
            x->val.apply = copyLamApply(o->val.apply);
            break;
        case LAMEXP_TYPE_IFF:
            x->val.iff = copyLamIff(o->val.iff);
            break;
        case LAMEXP_TYPE_CALLCC:
            x->val.callcc = copyLamExp(o->val.callcc);
            break;
        case LAMEXP_TYPE_LETREC:
            x->val.letrec = copyLamLetRec(o->val.letrec);
            break;
        case LAMEXP_TYPE_TYPEDEFS:
            x->val.typedefs = copyLamTypeDefs(o->val.typedefs);
            break;
        case LAMEXP_TYPE_LET:
            x->val.let = copyLamLet(o->val.let);
            break;
        case LAMEXP_TYPE_MATCH:
            x->val.match = copyLamMatch(o->val.match);
            break;
        case LAMEXP_TYPE_COND:
            x->val.cond = copyLamCond(o->val.cond);
            break;
        case LAMEXP_TYPE_AND:
            x->val.and = copyLamAnd(o->val.and);
            break;
        case LAMEXP_TYPE_OR:
            x->val.or = copyLamOr(o->val.or);
            break;
        ...
        case LAMEXP_TYPE_COND_DEFAULT:
            x->val.cond_default = o->val.cond_default;
            break;
        default:
            cant_happen("unrecognised type %d in copyLamExp", o->type);
    }
    x->type = o->type;
    UNPROTECT(save);
    return x;
...
struct LamCondCases * copyLamCondCases(struct LamCondCases * o) {
    if (o == NULL) return NULL;
    struct LamCondCases * x = NEW(LamCondCases, OBJTYPE_LAMCONDCASES);
    DEBUG("copy LamCondCases %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamCondCases));
    x->header = _h;
    int save = PROTECT(x);
    switch(o->type) {
        case LAMCONDCASES_TYPE_INTEGERS:
            x->val.integers = copyLamIntCondCases(o->val.integers);
            break;
        case LAMCONDCASES_TYPE_CHARACTERS:
            x->val.characters = copyLamCharCondCases(o->val.characters);
            break;
        default:
            cant_happen("unrecognised type %d in copyLamCondCases", o->type);
    }
    x->type = o->type;
    UNPROTECT(save);
    return x;
...
struct LamTypeConstructorType * copyLamTypeConstructorType(struct LamTypeConstructorType * o) {
    if (o == NULL) return NULL;
    struct LamTypeConstructorType * x = NEW(LamTypeConstructorType, OBJTYPE_LAMTYPECONSTRUCTORTYPE);
    DEBUG("copy LamTypeConstructorType %pn", x);
    Header _h = x->header;
    bzero(x, sizeof(struct LamTypeConstructorType));
    x->header = _h;
    int save = PROTECT(x);
    switch(o->type) {
        ...
        case LAMTYPECONSTRUCTORTYPE_TYPE_FUNCTION:
            x->val.function = copyLamTypeFunction(o->val.function);
            break;
        default:
            cant_happen("unrecognised type %d in copyLamTypeConstructorType", o->type);
    }
    x->type = o->type;
    UNPROTECT(save);
    return x;
