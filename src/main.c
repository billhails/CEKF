...
static void usage(char *prog, int status) {
    ...
    printf(
        ...
        "state table.\n"
        "    -l\n"
        "    --dump-lambda            Display all the lambda code.\n"
        "    -l<function>\n"
        "    --dump-lambda=<function> Display the lambda code for the "
        "function.\n"
        "    --dump-inline-c          Display the IR after "
        "constructor inlining.\n"
        "    -d\n"
        "    --dump-desugared         Display the IR after "
        "desugaring.\n"
        "    -d<function>\n"
        "    --dump-desugared=<function> Display the IR after "
        "desugaring.\n"
        "    -a\n"
        "    --dump-alpha             Display the IR after "
        "alpha-conversion.\n"
        "    -a<function>\n"
        "    --dump-alpha=<function>  Display the IR after "
        "alpha-conversion.\n"
        ...
        "Target B/C path specific dumps:\n"
        ...
        "    --dump-inline-f          Display the IR after function inlining.\n"
        "    --dump-closure           Display the IR after closure "
        "conversion.\n"
        "    --dump-ir                Display the final IR.\n"
        "    --dump-bytecode          Dump the generated bytecode (--terget-b "
        "only)\n"
        "Default path specific dumps:\n"
        "    --dump-ir                Display the final ANF.\n");
    exit(status);
}
    ...
    if (binary_input_file) {
        ...
        if (targetBFlag || targetCFlag) {
            ...
            while (previousInline != minExp) {
                ...
                // Inline
                //////////
                minExp = inlineMinExp(minExp, builtIns);
                REPLACE_PROTECT(save2, minExp);
